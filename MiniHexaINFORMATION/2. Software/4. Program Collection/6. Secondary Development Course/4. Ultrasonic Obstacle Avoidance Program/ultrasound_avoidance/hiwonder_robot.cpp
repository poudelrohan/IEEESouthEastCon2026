...
static Preferences preferences;      /* NVS non-volatile storage (NVS非易失储存) */
...
int8_t RobotJoint::read_deviation() {
  ...
  if (!preferences.begin(nvs_name.c_str(), true)) {
    ...
    preferences.begin(nvs_name.c_str(), false); // Open in write mode (以写模式打开)
...
bool RobotLeg::move(Vector_t point, uint32_t time) {
  ...
  if(fabs(goal_point.x - now_point.x) < 0.0001f &&
    ...
    is_busy = true;
    /* Forcefully write target value during motion (运动过程中强行写入目标值) */
...
bool Robot::next_circular_point(CircularPath* path, float* x, float* y) {
    ...
    /* All steps completed (including returning to the origin) (已完成所有步骤（包括回到原点）)*/
    ...
    /* Circular motion completed but not yet returned to the origin (已完成圆周运动，但还未返回原点)  */
...
void Robot::action_group_run(uint8_t id) {
  ...
  while(file.available()) {
    switch(act_state) {
      case READ_FRAME_NUM: /*Read frame header (读取帧头)*/
