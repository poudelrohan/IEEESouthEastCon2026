...
#define ULTRASOUND_ADDR                     0x77  /* Ultrasonic module I2C slave address (超声波模块I2C从机地址) */
#define DISTANCE_REG                        0     /* Distance lower 8 bits, unit: mm (距离低8位，单位mm) */
#define RGB_WORK_MODE_REG                   2     /*  RGB light mode setting register (RGB灯模式设置寄存器) */
...
class HW_Sensor{
  public:
    void begin();
    /*  */
    void set_ultrasound_rgb(uint8_t mode, uint8_t *rgb1, uint8_t *rgb2); //r1, g1, b1 indicate the breathing cycle of the right RGB light, 20 means 2s per cycle (r1，g1，b1表示右边rgb灯的呼吸周期，20表示2s一个周期)
    uint16_t _get_distance(void);
    uint16_t get_distance(void);
    uint16_t get_bat_voltage(void);
    uint8_t get_touch_state(void);  /* 0 - pressed, 1 - released (0-按下   1-松开)  */
    uint8_t get_ir1_state(void);    /*0 - obstacle present, 1 - no obstacle (0-有障碍 1-无)  */
