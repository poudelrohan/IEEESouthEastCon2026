...
void HW_Servo::transmit_packet(const uint8_t *data, uint8_t data_len) {
  /* Calculate total frame length (Frame header 2 + Type number 1 + Data length 1 + Data data_len + Checksum 1) (计算总帧长度（帧头2 + 类型号1 + 数据长度1 + 数据data_len + 校验1）) */
  uint8_t frame_len = 2 + 1 + 1 + data_len + 1;
  uint8_t frame[frame_len];

    /* Fill frame header (填充帧头)*/
    ...
    /* Data length (data_len) (数据长度（data_len）)*/
    ...
    /* Calculate checksum (frame header, function number, data length, data) (计算校验和（校验帧头、功能号、数据长度、数据）)*/
    uint8_t checksum = checksum_sum(frame, frame_len - 1);
    // /* Fill checksum (last byte) (填充校验位（最后一位）)*/
    frame[frame_len - 1] = checksum;
    // Serial.print(checksum);
    // Serial.println();
    /* Send frame via serial (通过串口发送帧) */
...
void HW_Servo::begin() {
  /* Servo driver (舵机驱动) */
  Serial1.setTxBufferSize(2048);  // Increase receive buffer to 2KB (增大接收缓冲区到2KB)
