...
/* Color thresholds, which are adjustable (颜色阈值，用户可在此调整)  */
...
/* Get detection result of color blocks (获取颜色检测的结果) */
static void get_color_detection_result(uint16_t *image_ptr, int image_height, int image_width, vector<color_detect_result_t> &results, uint16_t color)
{
  ...
  /* Find the largest connected area of the same color (寻找同色最大色块) */
...
static void task_process_handler(void *arg)
{
  ...
  /* Register color information (注册颜色信息)  */
