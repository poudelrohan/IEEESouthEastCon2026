...
// Moves drone away from nets at bottom-left corner
int SAFETY_FORWARD = 20;      // Move away from back net (min 20)
int SAFETY_RIGHT = 20;        // Move away from left net (min 20)

// ─────────────── DESCENT (cm) ───────────────
int DESCEND_AMOUNT = 60;      // How much to go down after takeoff

// ─────────────── MOVEMENT PATTERN (cm) ───────────────
// Adjust these to account for drift
int MOVE_RIGHT = 40;          // First move right
int MOVE_LEFT = 60;           // Then move left (more to compensate drift)

// ─────────────── TIMING (ms) ───────────────
int SLEEP_TIME = 10000;               // Sleep after first landing (10 sec)
int SEARCH_TIMEOUT = 15000;           // Max time to look for pad (15 sec)
...
// ─────────────── PAD LANDING ───────────────
int PAD_X_OFFSET = 10;        // cm forward from rocket icon
    ...
    while (millis() - searchStart < SEARCH_TIMEOUT) {
        ...
        if (elapsedSec != lastSecondPrinted) {
            ...
            if (padId > 0) {
                ...
                Serial.printf("Searching... %d seconds remaining\n", remainingSec);
            }
        }
        
        // Just hover - no movement
        stopMovement();
        delay(100);
    }
...
void runSimpleSequence() {
    ...
    // Step 7: Enable pad detection
    Serial.println("[7] Enabling pad detection...");
    ...
    // Step 9: Hover and search for pad
    Serial.println("[9] SEARCHING FOR PAD (hovering still)...");
    bool padFound = hoverAndSearchForPad();
    
    // Step 10: Land
    if (padFound) {
        Serial.println("\n[10] PAD FOUND - Centering and landing...");
        if (centerOnPad()) {
            descendAndLandOnPad();
        } else {
        ...
        Serial.println("\n[10] PAD NOT FOUND - Landing wherever...");
        performLand();
    }
