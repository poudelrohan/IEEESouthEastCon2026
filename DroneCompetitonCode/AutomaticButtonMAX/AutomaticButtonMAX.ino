// ══════════════════════════════════════════════════════════════
//  TELLO AUTOMATIC BUTTON MAX — COMPETITION TIMING VERSION
//
...
//    T=37-57s: Robot pressing red button + returning (20s)
//              Drone is in the air doing its tasks during this
//    T=57s+:   Robot is back, waiting for drone to land
//    T=~110s:  Drone lands (~80s flight time)
//    Robot waits ~53s after returning for drone
//
//  DRONE FLIGHT (~80s):
...
int RETURN_DISTANCE = 30;       // cm forward after turn (back toward pad)
...
void setup() {
    ...
    Serial.println("\n\n==========================================================");
    Serial.println("   TELLO AUTOMATIC BUTTON MAX — COMPETITION TIMING");
    Serial.println("==========================================================\n");
    ...
    Serial.println("  T=57s+:   Robot back, waiting ~53s for drone to land");
    Serial.println("  T=~110s:  Drone lands\n");
...

    {
        ...
        // [4] Forward 30 (come back toward pad)
        ...
        // [7] CENTER AT CURRENT HEIGHT (no descent yet)
        {
            for (int i = 0; i < 5; i++) { readState(); delay(100); }
            int currentZ = padZ;
