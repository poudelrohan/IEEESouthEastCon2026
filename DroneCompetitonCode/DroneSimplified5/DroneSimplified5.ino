...
int ROTATE_DEGREES = 270;      // Step 4: Rotate left (ccw)
...
// ─────────────── RC SEND RATE ───────────────
// How often to send RC commands (ms). Higher = less spam, more time for drone to react.
// v4 used 100ms (10 commands/sec). Start at 1000ms (1 command/sec) and tune from there.
int RC_INTERVAL_MS = 300;
...
// ══════════════════════════════════════════════════════════════
//                    CENTER ON PAD (NO TIMEOUT)
//
//  Same logic as v4 but:
//    - No 20s timeout — runs forever until centered or E/L
//    - RC commands sent every RC_INTERVAL_MS instead of 100ms
// ══════════════════════════════════════════════════════════════

bool centerOnPad() {
    Serial.println("\n───────── CENTERING ON PAD ─────────");
    Serial.printf("Target: X=%d, Y=%d (offset from rocket)\n", PAD_X_OFFSET, PAD_Y_OFFSET);
    Serial.printf("RC interval: %dms\n", RC_INTERVAL_MS);
    ...
    Serial.println("CENTERING ABORTED!");
    return false;
}

// ══════════════════════════════════════════════════════════════
//                    DESCEND AND LAND ON PAD (NO TIMEOUT)
//
//  Same logic as v4 but:
//    - No 20s timeout — runs forever until landed or E/L
//    - RC commands sent every RC_INTERVAL_MS instead of 100ms
// ══════════════════════════════════════════════════════════════
...
void setup() {
    ...
    Serial.println("\n\n══════════════════════════════════════════════════════════");
    Serial.println("      TELLO SIMPLE SEQUENCE v5 (RATE-LIMITED RC)");
    Serial.println("══════════════════════════════════════════════════════════\n");
    ...
    Serial.printf("  Land type:       %s\n", USE_EMERGENCY_LAND ? "EMERGENCY" : "Normal");
    Serial.printf("  RC interval:     %dms (change RC_INTERVAL_MS to tune)\n", RC_INTERVAL_MS);
    Serial.println();
