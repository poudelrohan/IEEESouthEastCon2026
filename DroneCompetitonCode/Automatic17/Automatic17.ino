// ══════════════════════════════════════════════════════════════
//  TELLO AUTOMATIC 17 — SINGLE FLIGHT, CENTER FIRST LANDING
//
//  Single flight version. No landing in between, no hovering.
//  Drone does everything in one go.
//
//  Changes from v16:
//    1. Single flight (no Phase 2 reposition/hover)
//    2. RETURN_DISTANCE = 30 (was 40, drone was overshooting)
//    3. Offsets confirmed correct via Tello forum research
//       (camera tilts ~10deg forward, GO_X_OFFSET=-5 compensates)
//    4. USE_EMERGENCY_LAND only affects the FINAL precision
//       landing — motors off drop after RC fine-tune at ~25cm.
//       Fallback landings always use normal land.
//
//  TIMELINE:
//    T=0-17s:  Robot delay (ESP connects WiFi + SDK)
//    T=17-29s: Robot moving to middle-left (12s)
//    T=29s:    Takeoff → fwd 40 → CW 180 → fwd 30 → CW 180
...
int RETURN_DISTANCE = 30;       // cm forward after turn (was 40, overshoots)

// ─────────────── GO COMMAND (gradual descent) ───────────────
int GO_HEIGHTS[] = {60, 50, 40, 30, 25};
int NUM_GO_STEPS = 5;
int GO_SPEED = 15;
int GO_X_OFFSET = -5;           // compensates ~10deg camera tilt
int GO_Y_OFFSET = -5;
...
// ─────────────── LANDING ───────────────
// When true: final precision landing uses emergency (motors off drop)
// Fallback landings ALWAYS use normal land regardless
...
// Normal land — used for fallback landings
void normalLand() {
    stopMovement();
    delay(200);
...
// Precision land — uses emergency (motors off) if USE_EMERGENCY_LAND is true
void precisionLand() {
    stopMovement();
    delay(200);
    if (USE_EMERGENCY_LAND) {
        Serial.println("*** EMERGENCY LAND (motors off drop) ***");
        sendCommand("emergency");
    } else {
        Serial.println("*** NORMAL LAND ***");
        sendCommand("land");
...
void setup() {
    ...
    Serial.println("\n\n==========================================================");
    Serial.println("   TELLO AUTOMATIC 17 — SINGLE FLIGHT");
    Serial.println("==========================================================\n");

    Serial.println("SEQUENCE:");
    Serial.println("  T=0-17s:  Robot delay (ESP connects)");
    Serial.println("  T=17-29s: Robot moving to middle-left");
    Serial.printf("  T=%lus:    Takeoff → fwd %d → CW 180 → fwd %d → CW 180\n",
                  TAKEOFF_TIME / 1000, MOVE_AWAY_DISTANCE, RETURN_DISTANCE);
    Serial.printf("            → find pad → center → descend → RC fine → %s\n\n",
                  USE_EMERGENCY_LAND ? "EMERGENCY DROP" : "normal land");
...

    {
        ...
        // [4] Forward 30 (return — shorter to not overshoot)
