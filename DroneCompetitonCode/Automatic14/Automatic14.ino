// ══════════════════════════════════════════════════════════════
//  TELLO AUTOMATIC 14 — TIME-SYNCED WITH ROBOT
//
//  Both boards power on at T=0. All drone actions are timed
//  to sync with the Arduino Mega robot sequence.
//
...
//    T=118s: Robot back at start. Begin pad detect + land
//
...
int batteryLevel = 0;
unsigned long bootTime = 0;     // millis() at start of setup
...
// Wait until a specific time (ms from power-on), with countdown
// Returns false if emergency triggered during wait
...
void setup() {
    ...
    Serial.println("\n\n==========================================================");
    Serial.println("   TELLO AUTOMATIC 14 -- TIME-SYNCED WITH ROBOT");
    Serial.println("==========================================================\n");
    ...
    Serial.printf("  T=%lus: Pad detect → go descent → RC fine-tune → land\n\n", LAND_PHASE_TIME / 1000);

    // ════════════════════════════════════════════════════════════
    // PHASE 0: CONNECT TO DRONE (during the first 15s shared delay)
    // ════════════════════════════════════════════════════════════
    ...
    //   T=27s: Robot just arrived at middle-left
    //   Forward 40 → CW 180 → Forward 40 → CW 180 → Land
    //   Lands facing original direction for phase 2
    // ════════════════════════════════════════════════════════════
...

    {
        ...
        // Re-enter SDK mode (in case it timed out while waiting)
        sendWithRetry("command", 500);
        sendWithRetry("mon", 500);
        sendWithRetry("mdirection 0", 500);
        ...
        // CW 180 (face original direction — so phase 2 doesn't need to rotate)
    ...
    //   Robot does crater (~30s) then starts heading back
    //   Drone takes off at T=89s, goes right 20, hovers in place
    //   No CW 180 needed — already facing original from phase 1
    // ════════════════════════════════════════════════════════════
...

    {
        ...
        // Already facing original direction from flight 1 CW 180
        // Right 20cm to position over robot's path
    ...
    // ════════════════════════════════════════════════════════════
    // PHASE 3: HOVER IN PLACE, THEN PRECISION LAND
    //   Wait until T=118s (robot back at start)
    //   Then find pad → go descent → RC fine-tune → land
    // ════════════════════════════════════════════════════════════

    if (!waitUntil(LAND_PHASE_TIME, "PRECISION LAND")) goto done;

    Serial.println("\n==========================================================");
    Serial.printf("T=%lus | PHASE 3: PRECISION LANDING\n", elapsedSeconds());
    Serial.println("==========================================================\n");

    {
        // Look for pad
        Serial.printf("T=%lus | [5] Looking for pad...\n", elapsedSeconds());
